...
export default function Chat() {
  ...
  const { applicants, selectedStudents, chatMessages, sendMessage } = useCompany();
  ...
  const contacts = allStudents.map((student, index) => ({
    ...
    email: student.email,
    lastMessage: chatMessages[student.id]?.slice(-1)[0]?.text || "Start a conversation...",
  ...
  if (!selectedContact) {
    return (
      <div className="max-w-7xl mx-auto h-[calc(100vh-180px)] flex items-center justify-center">
  ...
  return (
    <div className="max-w-7xl mx-auto h-[calc(100vh-180px)] flex flex-col">
      ...
      <div className="mb-4">
        ...
        <p className="text-gray-500 mt-1">Chat with applicants and candidates</p>
      </div>
      ...
      <div className="flex-1 flex bg-white rounded-2xl shadow-xl shadow-gray-200/50 overflow-hidden border border-gray-100">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <h4 className={`font-semibold truncate ${
                        selectedContact.id === contact.id ? "text-blue-600" : "text-gray-900"
                      }`}>
                        {contact.name}
                      </h4>
        ...
        <div className="flex-1 flex flex-col">
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-gray-50/80 to-white">
            {/* Date Divider */}
            ...
            {currentMessages.map((msg, index) => (
              <div
                key={msg.id || index}
                className={`flex ${msg.from === "company" ? "justify-end" : "justify-start"}`}
              >
                <div
                ...
                >
                  {/* File Attachment */}
                  {msg.file && (
                    <div className={`p-3 ${msg.text ? "border-b border-white/20" : ""}`}>
                      {msg.file.preview?.type === "image" ? (
                        ...
                        <div className={`flex items-center gap-3 p-3 rounded-lg ${
                          msg.from === "company" ? "bg-white/10" : "bg-gray-50"
                        }`}>
                          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                            msg.from === "company" ? "bg-white/20" : "bg-blue-100"
                          }`}>
                            <DocumentIcon className={`w-5 h-5 ${
                              msg.from === "company" ? "text-white" : "text-blue-600"
                            }`} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="font-medium text-sm truncate">{msg.file.name}</p>
                            <p className={`text-xs ${
                              msg.from === "company" ? "text-blue-100" : "text-gray-500"
                            }`}>{msg.file.size}</p>
                          </div>
                        </div>
                      )}
                  ...
                  <div
                    className={`flex items-center justify-end gap-1.5 px-4 pb-2.5 ${
                      msg.from === "company" ? "text-blue-100" : "text-gray-400"
                    }`}
                  >
          ...
          {/* File Preview */}
          {filePreview && (
            <div className="px-6 py-3 border-t border-gray-100 bg-gradient-to-r from-gray-50 to-white">
          ...
          <div className="px-6 py-4 border-t border-gray-100 bg-white">
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="p-2.5 hover:bg-gray-100 rounded-xl transition-colors group relative"
              >
              ...
              <div className="flex-1 relative">
                <input
                  ...
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 focus:bg-white transition-all pr-12"
                />
                ...
                <div className="absolute right-3 top-1/2 -translate-y-1/2">
                  ...
                  {/* Emoji Picker */}
                  {showEmojiPicker && (
                    <div className="absolute bottom-full right-0 mb-2 p-3 bg-white rounded-2xl shadow-xl border border-gray-100 grid grid-cols-8 gap-1 z-10">
                      {emojis.map((emoji) => (
                        <button
                          key={emoji}
                          onClick={() => {
        ...
        {showContactInfo && (
          <div className="w-80 border-l border-gray-100 bg-white p-6 overflow-y-auto">
            ...
            <div className="space-y-4">
              <button
                onClick={() => navigate(`/company/student/${selectedContact.id}`)}
